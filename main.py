...
==========================================================================
 ...
 Version: 3.1.0 (Dual Backend — Free + Paid)
 Date:    June 2026
==========================================================================
"""
...
NOMINATIM_USER_AGENT = "vehicle-routing-optimizer/3.1.0"
...
class OptimizeRouteResponse(BaseModel):
    ...
    geo_backend: str = Field(
        ...,
        description="Which geocoding/routing backend was used: 'osm' (free) or 'google' (paid).",
    )
    ...
    raw_matrix_seconds: List[List[int]] = Field(
        ..., description="The raw N×N travel-time matrix (seconds)."
    )
...
class HealthResponse(BaseModel):
    ...
    geo_backend: str = Field(
        ..., description="Active geocoding/routing backend: 'osm' or 'google'."
    )
...
async def lifespan(app: FastAPI):
    ...
    logger.info("🚀 Starting Vehicle Routing Optimizer API v3.1.0 …")
...
@app.get(
    ...
    description="Returns current server status, active backend, and ML model status.",
)
async def health_check() -> HealthResponse:
    ...
    **Health Check Endpoint**
    ...
    including the active geo backend and ML model load status.
    """
...
@app.post(
    ...
    description=(
        "Accepts a `LocationQuery` payload, geocodes addresses, "
        "builds a travel-time matrix, applies the XGBoost ML "
        "prediction overlay, and solves the optimal visit sequence via OR-Tools.\n\n"
        "Uses the backend configured by `GEO_BACKEND` env var "
        "(default: `osm` — free, no API key)."
    ),
    responses={
        400: {"description": "Invalid address or geocoding failure."},
        500: {"description": "Server configuration error (e.g. missing API key for Google backend)."},
...
async def optimize_route(payload: LocationQuery) -> OptimizeRouteResponse:
    ...
    **Full Route Optimization Pipeline**
    ...
    2. Build N×N travel-time + distance matrices (via OSRM or Google Distance Matrix)
    3. Apply XGBoost ML buffer overlay for weather/event anomalies
    4. Solve TSP/VRP via Google OR-Tools
    5. Return structured JSON with optimized sequence, durations, and matrices

    Raises:
        HTTPException 400: Invalid or unresolvable address.
        HTTPException 500: Missing API key (Google backend) or solver failure.
    ...
    raw_matrix, distance_matrix_meters = _build_matrices(
        locations, departure_dt
    )
...
def _build_matrices(
    ...
    """
    Build N×N time + distance matrices, dispatching to the
...
def _osm_geocode_single(
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_502_BAD_GATEWAY,
            detail=(
                f"Nominatim geocoding error for {label} ('{location}'): {exc}."
            ),
        )
...
def _osm_geocode_all(payload: LocationQuery) -> List[dict]:
    ...
    so we insert a small delay between calls.
    """
    locations = []
    all_items = (
...
def _osrm_build_matrices(locations: List[dict]) -> tuple:
    ...
    OSRM's /table/v1/driving endpoint returns duration and
    distance matrices for all coordinate pairs in a single request.
    ...
    Returns:
        ...
        Time in seconds (int), distance in meters (int).
    ...
    # OSRM expects coordinates as lon,lat (note: reversed from lat,lng)
    coords_str = ";".join(
        f"{loc['lng']},{loc['lat']}" for loc in locations
    )
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_502_BAD_GATEWAY,
            detail=(
                "OSRM routing API timeout. The public OSRM server "
    ...
    # Parse duration and distance matrices
    time_matrix = []
    for row in data["durations"]:
        time_matrix.append(
            [int(round(val)) if val is not None else 999999 for val in row]
        )
...
def _get_gmaps_client():
    ...
    variable.  If the key is missing, an HTTP 500 is raised.

    Returns:
        googlemaps.Client instance.

    Raises:
        HTTPException 500: if the API key is not set or package missing.
    """
    try:
        import googlemaps
    except ImportError:
...
def _google_geocode_all(gmaps, payload: LocationQuery) -> List[dict]:
    """Geocode all locations via Google Maps."""
    locations = []
    all_items = (
        [("Source", payload.source)]
        + [(f"Waypoint {i+1}", wp) for i, wp in enumerate(payload.waypoints)]
...
def _compute_next_departure(
    ...
    (Also used for logging context with the OSM backend.)
...
def _apply_ml_overlay(
    ...
    5. Floor guardrail: ``max(predicted_time, base_time)`` ensures
       no cell falls below the raw base value.

    If no anomaly conditions are present (no rain AND no festival),
    the raw matrix is passed through unchanged.
